...
public class ServerSessionStore {
    ...
    private static final long CLEANUP_INTERVAL_IN_MILLISECONDS = 1000 * 60 * 60 * 10; // 10 min.
    ...
    private SessionInfo createSessionInfoFor(User user, SessionInfo inactiveSession) {
        if ( !isInactiveSessionInfo(inactiveSession)) {
            return createNotLoggedInSession();
            // LOGGER.warn("Session is unknown to this server: {}", inactiveSession);
     ...
     * @deprecated use {@link #isActiveSessionInfo(SessionInfo)} and react with appropriate response
     */
    @Deprecated
